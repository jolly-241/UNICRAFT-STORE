...
const products = [
    ...
    },
    {
        ...
        price: 99,
        category: "jewelry",
        image: "assets/Resin_EarRings.jpeg",
        requiresText: false,
        desc: "Exquisite handmade silver-frame earrings filled with clear resin and dried magenta flower petals. Features a traditional metallic drop design with elegant silver beads.",
        isNew: true
    },
    {
...
function updateCartUI() {
    ...
    if (cart.length === 0) {
        ...
        checkoutBtn.href = getInstagramLink(encodedMessage);
...
function getInstagramLink(encodedMessage) {
    const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
    const instagramUsername = "unicraft_2k26";
    return isMobile 
        ? `https://ig.me/m/${instagramUsername}?text=${encodedMessage}`
...
function handleInstagramCheckout(e) {
    ...
    const instagramLink = getInstagramLink(encodedMessage);
...
function openCheckoutGuide() {
    ...
    // Adapt instruction texts dynamically for mobile vs desktop
    const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
    const stepOneText = document.querySelector(".checkout-steps .step:first-child p");
    if (stepOneText) {
        if (isMobile) {
            stepOneText.innerHTML = `We are opening Instagram DM with <strong>@unicraft_2k26</strong>`;
        } else {
            stepOneText.innerHTML = `We are opening the Instagram Profile of <strong>@unicraft_2k26</strong> (Click the "Message" button on their profile)`;
        }
    }
